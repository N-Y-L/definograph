...
import { DistanceProfileView } from './visual/DistanceProfileView';
...
function BallView({scene,scenario,onVariableChange}: {scene:BallScene;scenario:Scenario;onVariableChange:(id:string,value:ScenarioValue)=>void}) {
  ...
  const [fixedCoordinate,setFixedCoordinate] = useState(2);
  const omittedCoordinates = fixed.map((_,index) => index).filter(index => !axes.includes(index));
  const effectiveFixedCoordinate = omittedCoordinates.includes(fixedCoordinate) ? fixedCoordinate : omittedCoordinates[0];
  ...
  return <div className="scene-view">
    <div className="plot-meta"><span className="metric-tag">{metricLabel(scene.metric)}</span><span>{scene.dimension > 2 ? `Ambient dimension ${scene.dimension} · ${mode === 'profile' ? 'distance profile' : 'coordinate slice'}` : scene.dimension === 1 ? 'Real line' : 'Full two-dimensional view'}</span></div>
