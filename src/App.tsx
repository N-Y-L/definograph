...
export default function App(){
  ...
  async function analyze(text=source,inputMode=mode,expansions=expandNames){
    ...
    try{
      ...
      setExperience('read');setInspectorTab('objects');
  ...
  function exportDocument(){if(!document||!plan)return;const blob=new Blob([JSON.stringify({format:'statementlens-workspace',version:2,reading,experience,analysis:checked,viewMode:inspectBody&&checked?.definitionTree?'definition-body':'original',document,plan,scenario},null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=window.document.createElement('a');a.href=url;a.download='statementlens-workspace.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-icon">∈</span><span>Statement<span className="brand-light">Lens</span></span><span className="version">STATEMENT VISUALIZER</span></div><div className="top-actions"><span className="local-label"><i/>Local · Lean 4</span><button className="quiet-button" onClick={()=>setGuide(!guide)} aria-expanded={guide}>Reading the diagrams</button><button className="toolbar-button" onClick={exportDocument} disabled={!current}>Export workspace ↗</button></div></header>
    {guide&&<section className="guide"><button className="close-guide quiet-button" onClick={()=>setGuide(false)}>Close</button><h2>Read a statement, one visual step at a time.</h2><div className="guide-grid"><p><strong>A sequence with an overview.</strong> Follow the objects, conditions, and conclusion while the full logical structure stays alongside. Objects retain their identity across diagrams. Select an object to inspect its type and the fragments where it appears.</p><p><strong>Quantifiers describe choices.</strong> ∀ introduces an arbitrary choice; ∃ asks for a witness. Dependency arrows show which earlier choices a witness may use.</p><p><strong>Abstraction comes first.</strong> Objects and maps need no coordinates. Diagrams show the relationships supplied by the statement; recognized geometric objects can also receive symbolic geometry.</p><p><strong>Exploration is optional.</strong> Open Explore a sample to choose numerical values. The reading sequence does not depend on those choices. Elaboration alone never establishes truth.</p></div></section>}
    <main className="workspace workbench">
      ...
      <section className="visual-panel">
        <div className="canvas-heading"><div className="eyebrow">VISUAL READING</div>{current&&<span className="status-tag">{checked?.definitionTree&&inspectBody?'Definition body from Lean':typedAnalysis?.validation?.includes('declaration')?'Signature checked by Lean':'Statement type checked by Lean'}</span>}</div>
        {current&&document&&reading&&plan&&primary?<>
          <div className="visual-title"><div><div className="canvas-breadcrumb"><button onClick={()=>chooseNode(current.tree.id)}>Statement</button>{selected!==current.tree.id&&<><span>/</span><span>{selectedNode?.binder?.name??selectedNode?.kind}</span></>}</div><h1>{experience==='read'?'Read the statement':primary.title}</h1></div><span className="auto-badge">Automatic composition</span></div>
          ...
          <div className="experience-tabs" aria-label="Reading and exploration"><button className={experience==='read'?'active':''} aria-pressed={experience==='read'} onClick={()=>{setExperience('read');setInspectorTab('objects');}}>Statement sequence</button><button className={experience==='explore'?'active':''} aria-pressed={experience==='explore'} onClick={()=>{setExperience('explore');setInspectorTab(primary.fidelity==='numerical'?'scenario':'objects');}}>Explore a sample <span>Optional</span></button><button className="notation-toggle" aria-pressed={showNotation} onClick={()=>setShowNotation(!showNotation)}>{showNotation?'Hide notation':'Show mathematical notation'}</button></div>
          {showNotation&&<Suspense fallback={<p>Loading mathematical notation…</p>}><MathematicalStatement notation={inspectBody&&checked?.definitionTree?checked.definitionReadableMath:checked?.readableMath} lean={current.pretty}/></Suspense>}
          {experience==='read'?<StatementReadingView reading={reading} document={document} selectedObjectId={objectId} onObjectSelect={id=>{setObjectId(id);setInspectorTab('objects');}} onNodeSelect={chooseNode} renderGeometry={panel=>{
          ...
          }}/>:<><p className="exploration-intro">Optional illustrations of selected fragments. Numerical values do not change the statement or establish its truth.</p>
          ...
          <details className="lean-details"><summary>{selected===current.tree.id?(checked?.definitionTree&&inspectBody?'Definition body':typedAnalysis?.validation?.includes('declaration')?'Typed signature':'Elaborated statement'):'Selected fragment · enclosing scope retained'}</summary><pre>{selectedNode?.lean}</pre></details>
    ...
    </main><footer><span>StatementLens <span className="footer-divider">/</span> Mathematical statements, made visible</span><span>Codex under the supervision of Neil Yuanting Li</span></footer>
  </div>;
}
