...
export default function App(){
  ...
  useEffect(()=>{fetch('/api/health').then(r=>r.json()).then(setHealth).catch(()=>setHealth({ready:false,issue:'Local Lean service is unavailable.'}));void analyze(examples[0].source,'term',[],false);return()=>active.current?.abort();},[]);
  ...
  const title=mode==='declaration'?checked?.provenance?.declaration?.name??source.trim():activeExample?.title??'Your mathematical statement';
  return <div className="app-shell">
    <a className="skip-link" href="#statement-reading">Skip to the statement</a>
    <header className="topbar"><a href="#statement-reading" className="brand" aria-label="StatementLens home"><span className="brand-icon" aria-hidden="true">∈</span><span>Statement<span className="brand-light">Lens</span></span></a><span className="brand-purpose">A visual language for mathematics</span><nav className="top-actions" aria-label="Workspace tools"><span className="local-label"><i className={health?.ready?'ready':''}/>{health?.ready?'Lean connected':'Connecting to Lean'}</span><button className="quiet-button" onClick={()=>setDrawer('guide')}>Guide</button><button className="toolbar-button" onClick={exportDocument} disabled={!current}>Export</button></nav></header>
    ...
    <main className="workspace atlas-workspace">
      <div className="statement-bar"><div className="statement-picker"><label htmlFor="example">Statement</label><select id="example" value={mode==='term'?exampleId:''} onChange={e=>loadExample(e.target.value)} disabled={busy}><option value="" disabled>{mode==='declaration'?'Lean declaration':'Custom Lean statement'}</option>{examples.map(e=><option key={e.id} value={e.id}>{e.title}</option>)}</select></div><div className="statement-actions"><button className="toolbar-button" onClick={()=>setDrawer('source')}>Lean source <span aria-hidden="true">↗</span></button><button className="toolbar-button" onClick={()=>setDrawer('inspect')} disabled={!current}>Inspect <span aria-hidden="true">↗</span></button></div></div>
      ...
      <section className="visual-panel" id="statement-reading" aria-label="Statement reading">
        ...
        {current&&document&&reading&&plan&&primary?<>
          <div className="visual-title"><div className="atlas-kicker"><span>THE VISUAL STATEMENT</span><span className="status-tag">{checked?.definitionTree&&inspectBody?'Definition body':typedAnalysis?.validation?.includes('declaration')?'Signature checked by Lean':'Type checked by Lean'}</span></div><h1>{title}</h1><p>{activeExample?.description??'Read the objects and relationships in their logical context.'}</p></div>
        ...
        </>:<div className="empty-view"><div className="empty-orbit"><span>∀</span><i>∈</i></div><h1>{busy?'Reading the mathematical structure':analysis?'Your source has changed':'From a statement to its structure'}</h1><p>{busy?'Lean is resolving types, scope, and definitions. The first analysis starts the local environment.':error?'Open Lean source to review and correct the input.':'Interpret the current Lean input to build connected views of its objects, relationships, and choices.'}</p>{!busy&&<button className="toolbar-button" onClick={()=>setDrawer('source')}>{error?'Review Lean source':'Open Lean source'}</button>}{error&&<p className="error-summary" role="alert">The input could not be read.</p>}</div>}
      </section>
      <Drawer open={drawer==='inspect'} title="Inspect the statement" onClose={()=>setDrawer(null)}><section className="scenario-panel inspector">
        ...
        {current&&document&&plan&&<div className="inspection-details">          <details className="lean-details"><summary>{selected===current.tree.id?(checked?.definitionTree&&inspectBody?'Definition body':typedAnalysis?.validation?.includes('declaration')?'Typed signature':'Elaborated statement'):'Selected fragment · enclosing scope retained'}</summary><pre>{selectedNode?.lean}</pre></details>
    ...
    </main><footer><span>StatementLens <span className="footer-divider">/</span> Read the mathematics</span><span>Codex under the supervision of Neil Yuanting Li</span></footer>
  </div>;
}
