...
export default function App(){
  ...
  function exportDocument(){if(!document||!plan)return;const blob=new Blob([JSON.stringify({format:'statementlens-workspace',version:3,reading,cues,editor:editorSession?.document,experience,analysis:checked,viewMode:inspectBody&&checked?.definitionTree?'definition-body':'original',document,plan,scenario},null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=window.document.createElement('a');a.href=url;a.download='statementlens-workspace.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
  ...
  return <div className="app-shell">
    ...
    <header className="topbar"><a href="#statement-reading" className="brand" aria-label="StatementLens home"><span className="brand-icon" aria-hidden="true">∈</span><span>Statement<span className="brand-light">Lens</span></span></a><span className="brand-purpose">A visual language for mathematics</span><nav className="top-actions" aria-label="Workspace tools"><span className="local-label"><i className={health?.ready?'ready':''}/>{host?'Lean editor':health?.ready?'Lean connected':'Connecting to Lean'}</span><button className="quiet-button" onClick={()=>setDrawer('guide')}>Guide</button><button className="toolbar-button" onClick={exportDocument} disabled={!current}>Export</button></nav></header>
