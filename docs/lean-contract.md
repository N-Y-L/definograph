...
6. Export the logical tree, typed object descriptors, context, source occurrences, and imported definition metadata. Optional definition expansion runs only on explicitly named safe imported definitions and checks each result with the kernel and definitional equality.
